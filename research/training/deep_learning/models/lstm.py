...
class BiomeLSTM(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, num_layers: int, output_size: int, dropout: float = 0.2,
        ...
        # TODO: Inicializar pesos attention?
        self.attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=num_heads, batch_first=True)

        self.pre_layer_norm = nn.LayerNorm(hidden_size)
    ...
    def forward(self, x: torch.Tensor, hidden: Optional[Tuple[torch.Tensor, torch.Tensor]] = None) -> Tuple[
        ...
        lstm_out_norm = self.pre_layer_norm(lstm_out)
        # attn_output shape: (batch_size, sequence_length, hidden_size)
        attn_output, _ = self.attention(lstm_out_norm, lstm_out_norm, lstm_out_norm)

        # Residual! por fin, las llevo viendo desde AARN
        out = self.layer_norm(lstm_out + attn_output)
        # Hago pooling tomo la media sobre la dimensión temporal de seq
        out = out.mean(dim=1)

        # Salida lineal: (batch_size, output_size * target_horizon)
        out = self.fc(out)
        out = out.view(batch_size, self.target_horizon, self.output_size)
