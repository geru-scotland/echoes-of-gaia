...
from shared.enums.enums import FaunaAction, Agents, Direction, BiomeType
...
class FaunaAgentAI(Agent[Observation, FaunaAction]):
    def __init__(self, fauna_model: str, entity_provider: EntityProvider, worldmap_manager: WorldMapManager,
        ...
        self._foraging_system = ForagingSystem(worldmap_manager)
    ...
    def act(self, actions: Dict[int, FaunaAction]) -> None:
        ...
        for entity_id, action in actions.items():
            try:
                ...
                self._foraging_system.set_target(fauna_entity)
                ...
                if fauna_entity.energy_reserves > 0:
                    ...
                    (self._logger.debug(f"Post positin {fauna_entity.get_position()}"))
    ...
    def _prepare_entity_observation(self, entity: Fauna) -> Dict[str, Any]:
        ...
        fauna_map = np.zeros((fov_size, fov_size), dtype=np.int8)

        if position:
            local_maps = self._worldmap_manager.get_local_maps(position, fov_size, fov_size)
            if local_maps:
                terrain_map, validity_map, flora_map, fauna_map = local_maps

                terrain_map = terrain_map.astype(np.int64)
                validity_map = validity_map.astype(np.float32)
                flora_map = flora_map.astype(np.float32)
                fauna_map = fauna_map.astype(np.float32)
